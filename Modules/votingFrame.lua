...
function RCVotingFrame:OnCommReceived(prefix, serializedMsg, distri, sender)
	if prefix == "RCLootCouncil" then
		...
		if not decoded or decoded == nil then
			return
		end
		local decompressed = Deflate:DecompressDeflate(decoded)
		if not decompressed or decompressed == nil then
			return
		end
		local test, command, data = addon:Deserialize(decompressed)

		if not data or #data == 0 or data == nil then
			return
		end
...
function RCVotingFrame:GetCandidateData(session, candidate, data)
	...
	if not ok then
		addon:Debug("Error in 'GetCandidateData':", arg, session, candidate, data)
	else
		return arg
	end
end
...
function RCVotingFrame:SwitchSession(s)
	...
	sessionButtons[old] = self:UpdateSessionButton(old, lootTable[old].texture, lootTable[old].link,
		lootTable[old].awarded)
	...
	if nameCheck and #lootDB[name] > 0 then -- they're in the DB!
		...
		for i = #lootDB[name], 1, -1 do -- Start from the end
			...
			if not historical_item_slot or historical_item_slot == "" then
				...
				if not addon.Slots_INVTYPE[item_slot] then -- when we are comparing a normal item with a token we won previously
					...
					item_slot = addon.INVTYPE_Slots[original] and addon.INVTYPE_Slots[original][1] or
						addon.INVTYPE_Slots[original] or "INVTYPE_NONE"
			...
			if entry.responseID == addon.db.profile.checkID or 1 and not entry.isAwardReason and item_slot == historical_item_slot and not hasWonMainspec then -- Won MS roll for this slot
				tip:AddDoubleLine(format(L["Item won for 'roll':"], addon:GetResponseText(entry.responseID)), "", 1, 1, 1,
					1, 1, 1)
				tip:AddLine(entry.lootWon)
				tip:AddDoubleLine(entry.time .. " " .. entry.date,
					format(L["'n days' ago"], addon:ConvertDateToString(addon:GetNumberOfDaysFromNow(entry.date))), 1, 1,
					1, 1, 1, 1)
				tip:AddLine(" ") -- Spacer
				hasWonMainspec = true
			else
			...
			if not color[entry.response] then -- If it's not already added
				color[entry.response] = entry.color and #entry.color == 4 and entry.color or
					addon:GetResponseColorTable(entry.responseID) or { 1, 1, 1, 1 }
			end
		end                  -- end counting
		...
		if not hasWonMainspec then -- list non mainspec entries if we havent won a mainspec item
			...
			for _, entry in ipairs(nonMainspecEntries) do
				...
				tip:AddDoubleLine(format(L["Won 'item'"], entry.lootWon), addon:GetResponseText(entry.responseID), 1, 1,
					1, r, g, b)
				tip:AddDoubleLine(entry.time .. " " .. entry.date,
					format(L["'n days' ago"], addon:ConvertDateToString(addon:GetNumberOfDaysFromNow(entry.date))), 1, 1,
					1, 1, 1, 1)
...
function RCVotingFrame:GetFrame()
	...
	if addon.isMasterLooter then
		b1:SetScript("OnClick",
			function() if active then LibDialog:Spawn("RCLOOTCOUNCIL_CONFIRM_ABORT") else self:Hide() end end)
	else
...
function ResponseSort(table, rowa, rowb, sortbycol)
	...
	a, b = addon:GetResponseSort(lootTable[session].candidates[a.name].response),
		addon:GetResponseSort(lootTable[session].candidates[b.name].response)
	if a == b then
